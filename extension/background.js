  ...
  return new Promise((resolve) => {
    try {
      chrome.tabs.sendMessage(tabId, message, { frameId: 0 }, (response) => {
        ...
        resolve({
          response: runtimeError ? null : response,
...
async function sendReadinessWithContentScriptRecovery(tabId, message) {
  let channel = await sendTopFrameMessage(tabId, message);
  if (!channel.error) return channel;

  const missingReceiver = channel.error.includes("Receiving end does not exist");
  if (!missingReceiver || !chrome.scripting?.executeScript) return channel;
  ...
  return sendTopFrameMessage(tabId, message);
}
...
async function dispatchSingleReceiptInner(receiptRecord, stored, profileId) {
  ...
  const readinessChannel = await sendReadinessWithContentScriptRecovery(tabId, {
    action: "check_delivery_readiness",
    expectedConversationId: originConversationId
  });
  if (readinessChannel.error) {
    return { status: "waiting", reason: "tab_not_ready", message: readinessChannel.error };
  }
  const readinessResp = readinessChannel.response;

  if (!readinessResp || !readinessResp.ok || !readinessResp.documentId) {
    const waitReason = readinessResp?.readiness?.reason || "readiness_guards_not_passed";
    return {
      status: "waiting",
      reason: waitReason,
  ...
  let verifyResp = null;
  for (let i = 0; i < 3; i++) {
    const verifyChannel = await sendTopFrameMessage(tabId, {
      action: "verify_submitted_message",
      receiptMarker,
      expectedConversationId: originConversationId
    });
    verifyResp = verifyChannel.response;
